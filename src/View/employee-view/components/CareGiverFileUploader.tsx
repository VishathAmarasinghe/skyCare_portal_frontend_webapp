...
const CareGiverFileUploader = ({
  ...
  useEffect(() => {
    ...
    if (careGiverSlice?.selectedCareGiver && careGiverSlice.selectedCareGiver.careGiverDocuments.length > 0) {
      const updatedDocuments = createdDocArray.map((doc) => {
        const foundDocument = careGiverSlice?.selectedCareGiver?.careGiverDocuments.find(
          (careGiverDoc) => careGiverDoc.documentTypeID === doc.documentTypeID
        );
        return foundDocument
          ? { ...doc, uploadedDocument: foundDocument.document, expirationDate: foundDocument.expDate }
          : doc;
      });
