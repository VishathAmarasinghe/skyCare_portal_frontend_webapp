...
const ClientPastAppointmentCard = ({ appointment }: TimeSheetCardProps) => {
...
const ClientPendingAppointmentCardList = ({ }: TimeSheetTableProps) => {
  ...
  const appointmentSlice = useAppSelector((state) => state?.appointments);
    ...
    useEffect(() => {
      ...
      setRows(mappedRows || []);
    }, [appointmentSlice?.pastAppointmentToClient,appointmentSlice?.state]);

  return (
    <Stack sx={{ width: "100%", padding: 2 }}>
      {rows.map((appointment, index) => (
        <Box key={index} sx={{ marginBottom: 2 }}>
        <ClientPastAppointmentCard key={index} appointment={appointment} />
        </Box>
      ))}
      {
        rows.length === 0 && (
