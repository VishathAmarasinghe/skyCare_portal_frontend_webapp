...
const NextAppointmentCard = ({
  ...
  const [nextAppointment, setNextAppointment] =
    React.useState<NextAppointmentDTO | null>(null);
  const shiftNotesSlice = useAppSelector((State) => State.shiftNotes);
  const [isShiftNoteStarted, setIsShiftNoteStarted] =
  ...
  const handleShiftStarted = () => {
    console.log(
      "shiftNote Availability ",
      shiftNotesSlice?.currentShiftNoteState?.shiftNoteAvailability
    );
    console.log(
      "Shift State ",
      shiftNotesSlice?.currentShiftNoteState?.shiftNoteState
    );
    console.log(
      "Employee ID ",
      shiftNotesSlice?.currentShiftNoteState?.employeeID
    );
    ...
    console.log(
      ...
      shiftNotesSlice?.currentShiftNoteState?.shiftNoteAvailability ==
        "available" &&
        shiftNotesSlice?.currentShiftNoteState?.shiftNoteState == "Started" &&
        shiftNotesSlice?.currentShiftNoteState?.employeeID == authUser?.userID
    );
  };
    ...
    >
      {!nextAppointment?.recurrentTask ? (
        ...
        >
          ...
          {/* Add the icon */}
          <Typography variant="h6" color="textSecondary" sx={{ width: "100%" }}>
        ...
        <>
          ...
          >
            ...
            <Stack
              direction={{ sx: "row", sm: "row", md: "column", lg: "row" }}
              spacing={1}
            >
              <Button
                variant="outlined"
          ...
          <Stack spacing={2}>
            <Stack direction="row" alignItems="center">
              ...
              <Typography variant="body2" ml={1}>
                <strong>Start:</strong>{" "}
                {dayjs(nextAppointment?.recurrentTask?.startDate).format(
                  "MMM D, YYYY"
                )}{" "}
                {dayjs(
                  nextAppointment?.recurrentTask?.startTime,
                  "HH:mm"
                ).format("h:mm A")}
              </Typography>
            </Stack>
            <Stack direction="row" alignItems="center">
              ...
              <Typography variant="body2" ml={1}>
                <strong>End:</strong>{" "}
                {dayjs(nextAppointment?.recurrentTask?.endDate).format(
                  "MMM D, YYYY"
                )}{" "}
                {dayjs(nextAppointment?.recurrentTask?.endTime, "HH:mm").format(
                  "h:mm A"
                )}
              </Typography>
            </Stack>
            <Stack direction="row" alignItems="center">
              ...
              <Typography variant="body2" ml={1}>
                ...
                {nextAppointment?.recurrentTask?.comment ||
