...
const IncidentForm: React.FC<IncidentFormProps> = ({
  ...
  const [involvedPartiesRows, setInvolvedPartiesRows] = useState<
    IncidentInvolvedParties[]
  >([]);
  ...
  useEffect(() => {
    if (incidentSlice?.incidentsTypes) {
      setIncidentTypes(
        incidentSlice?.incidentsTypes?.filter(
          (type: IncidentType) => type.status === "Active"
        )
      );
    }
  ...
  useEffect(() => {
    if (incidentSlice?.incidentStatus) {
      setIncidentStatus(
        incidentSlice?.incidentStatus?.filter(
          (status: IncidentStatus) => status.status === "Active"
        )
      );
    }
      ...
      {({
        ...
        useEffect(() => {
          if (addressDetails) {
            ...
            setFieldValue("address.postalCode", addressDetails.postalCode);
          }
        }, [addressDetails]);
                  ...
                  <Grid item xs={12} sm={4} md={4}>
                    ...
                    >
                      {incidentTYpes.map((type: IncidentType) => (
                        <MenuItem value={type.incidentTypeID}>
                          {type.title}
                        </MenuItem>
                      ))}
                    </TextField>
                  ...
                  <Grid item xs={12} sm={4} md={4}>
                    ...
                    >
                      {incidentStatus.map((status: IncidentStatus) => (
                        <MenuItem value={status.incidentStatusID}>
                          {status.activeStatus}
                        </MenuItem>
                      ))}
                    </TextField>
