...
import { TextField,Typography, Button, ToggleButton, ToggleButtonGroup, Grid, Box, Stack } from '@mui/material';
import { Save as SaveIcon, Cancel as CancelIcon } from '@mui/icons-material';
import { ClientClassification, ClientType, Language, saveClassification, saveClientStatus, saveClientType, SaveLanguage } from '@slices/selectorSlice/selector';
import { useAppDispatch, useAppSelector } from '@slices/store';
import { enqueueSnackbar } from 'notistack';
import { State } from '../../../types/types';
import { LoadingButton } from '@mui/lab';
import { enqueueSnackbarMessage, ShowSnackBarMessage } from '@slices/commonSlice/common';
...
interface SupportAddingsProps {
  ...
  setSupportTypeCreater: React.Dispatch<React.SetStateAction<{
    supportType: 'language' | 'classification' | 'clientType' | 'clientStatus';
    isOpen: boolean;
  }>>;
}

const SupportAddings: React.FC<SupportAddingsProps> = ({ supportTypeCreater, setSupportTypeCreater }) => {
  ...
  const handleStateChange = (event: React.MouseEvent<HTMLElement>, newState: string) => {
    if (newState !== null) {
  ...
  const handleCreate = () => {
    if (supportTypeCreater.supportType==="classification") {
        ...
        if (payload.classificationName.trim() != '') {
            ...
            enqueueSnackbar({message:"Classification Name is required",variant:"error",anchorOrigin:{vertical:"bottom",horizontal:"right"}});
        }
        ...
        if (payload.name.trim() != '') {
            ...
            enqueueSnackbar({message:"Client Type is required",variant:"error",anchorOrigin:{vertical:"bottom",horizontal:"right"}});
        }
        ...
        if (payload.language.trim() != '') {
            ...
            enqueueSnackbar({message:"Language is required",variant:"error",anchorOrigin:{vertical:"bottom",horizontal:"right"}});
        }
        ...
        if (payload.status.trim() != '') {
            ...
            enqueueSnackbar({message:"Client Status is required",variant:"error",anchorOrigin:{vertical:"bottom",horizontal:"right"}});
        }
    }
  };
  ...
  // Handle form cancel
  const handleCancel = () => {
    setFormData({ name: '', languageNotes: '', classificationName: '', clientType: '', state: 'Inactive' });
  ...
  return (
    <Stack display={supportTypeCreater.isOpen?"flex":"none"} p={2} flexDirection="column"  justifyContent={"center"} justifyItems={"center"}
    sx={{border: "1px solid #ccc", borderRadius: 1,mt:2}}
    >
        <Typography align='left' variant='h6' fontWeight={600} color='primary'>Add New {supportTypeCreater.supportType}</Typography>
