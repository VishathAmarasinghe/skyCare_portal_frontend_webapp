...
const AddNewNotesModal = ({isNoteModalVisible,setIsNoteModalVisible,isEditMode,setIsEditMode}:AddNewNotesModalProps) => {
  ...
  return (
    ...
    title={
      <Box display="flex" justifyContent="space-between" alignItems="center">
        <Typography variant="h6">{noteState?.selectedNote?"Update":"Add"} Notes</Typography>
        {
    ...
    >
      <Box sx={{ mt: 2 }}  width="100%">
        <AddNoteForm isEditMode={isEditMode} isNoteModalVisible={isNoteModalVisible} key="note-Add-form"/>
      </Box>
    </Modal>
  )
}
